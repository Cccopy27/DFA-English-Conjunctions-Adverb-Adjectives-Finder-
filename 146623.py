...
fileuploadP = [psg.Input(key='-FILEBROWSEP-', focus=False, enable_events=True), psg.FileBrowse(target='-FILEBROWSEP-')]

#input text
inputT=psg.Text("Text Input")
fileuploadTT=psg.Text('Select a file to open:')
fileuploadT = [psg.Input(key='-FILEBROWSET-', focus=False, enable_events=True), psg.FileBrowse(target='-FILEBROWSET-')]
    ...
    for layer in wordlist:
        ...
        for i, char in enumerate(layer):
            if char not in current_dict:  # check for whether the dfa got output for current state when input = char 
                ...
                if (i<len(layer)-1): # havent reach the accepting state
                    current_dict[char] = {'-2':{'-2':'-2'}} # assign trap state

                else: # reach the accepting state
                    current_dict[char] = {'-1':'-1', '-2':{'-2':'-2'}} # assign accept state and trap state 
